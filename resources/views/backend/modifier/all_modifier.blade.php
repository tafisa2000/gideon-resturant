        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">Modifiers</h4>
                </div>
            </div>
        </div>
                            ...
                            <tbody>
                                @foreach ($modifiers as $key => $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->name }}</td>
                                        <td>
                                            <a href="{{ route('edit.modifier', $item->id) }}" class="btn btn-blue rounded-pill waves-effect waves-light edit-btn" 
                                               data-id="{{ $item->id }}" data-name="{{ $item->name }}" 
                                               data-bs-toggle="modal" data-bs-target="#edit">Edit</a>
                                            <a href="{{ route('delete.modifier', $item->id) }}" class="btn btn-danger rounded-pill waves-effect waves-light">Delete</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <form class="px-3" method="post" action="{{ route('modifier.store') }}">
                    @csrf
                    <div class="mb-3">
                        <label for="category_name" class="form-label">Name</label>
                        <input class="form-control" type="text" name="modifier" placeholder="" required>
                    </div>
...
<div id="edit" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <form class="px-3" method="post" action="{{ route('modifier.update') }}">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" value="" id="edit-modifier-name" required>
                    </div>
                    <div class="text-end">
